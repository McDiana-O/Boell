	...
	void Update () 
	{
		if (activateAnimacion) {
			if (animaDown == true) {
				...
				thisObjet.material.color = new Color (1f, 1f, 1f, (thisObjet.material.color.a - (2.0f / waitTime * Time.deltaTime)));
			} else {
				thisObjet.material.color = new Color (1f, 1f, 1f, (thisObjet.material.color.a + (2.0f / waitTime * Time.deltaTime)));
			}

			if (thisObjet.material.color.a < 0.0) {
				animaDown = false;
			} else if (thisObjet.material.color.a > 0.9) {
				animaDown = true;
			}
		}
	}
